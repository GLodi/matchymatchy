...
class LogicProviderImpl implements LogicProvider {
  ...
  Future<GameField> applyMove(GameField gameField, Move move) async {
    switch(move.dir) {
      case 0: { // up
        ...
        gameField.grid = gameField.grid.replaceRange(move.from, move.from+1, other);
        gameField.grid = gameField.grid.replaceRange(move.from-5, move.from-4, toMove);
        return gameField;
      }
      case 1: { // right
        var toMove = gameField.grid[move.from];
        ...
        gameField.grid = gameField.grid.replaceRange(move.from, move.from+1, other);
        gameField.grid = gameField.grid.replaceRange(move.from+1, move.from+2, toMove);
        return gameField;
      }
      case 2: { // down
        var toMove = gameField.grid[move.from];
        var other = gameField.grid[move.from+5];
        gameField.grid = gameField.grid.replaceRange(move.from, move.from+1, other);
        gameField.grid = gameField.grid.replaceRange(move.from+5, move.from+6, toMove);
        return gameField;
      }
      case 3: { // left
        var toMove = gameField.grid[move.from];
        var other = gameField.grid[move.from-1];
        gameField.grid = gameField.grid.replaceRange(move.from, move.from+1, other);
        gameField.grid = gameField.grid.replaceRange(move.from-1, move.from, toMove);
        return gameField;
      }
  ...
  @override
  Future<bool> checkIfCorrect(GameField gameField, TargetField targetField) async {
    bool result = true;
