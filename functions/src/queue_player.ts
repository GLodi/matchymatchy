 ...
 */
async function reconnect(userId: string, userFcmToken: string, currentMatch: string): Promise<Session> {
...
async function queueNotEmpty(userId: string, userFcmToken: string): Promise<[DocumentSnapshot, string]> {
    ...
    let matchId: string =
        await delQueueStartMatch(query.docs[0], userId, userFcmToken)
    let match: DocumentSnapshot =
        await matches.doc(matchId).get()
    users.doc(userId).update({
